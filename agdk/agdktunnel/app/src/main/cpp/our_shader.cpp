...
void OurShader::Compile() {
    ...
    if (mColorLoc < 0) {
        ALOGE("*** Couldn't get color attrib location from shader (OurShader).");
        ABORT_GAME;
    }
    mTintLoc = glGetUniformLocation(mProgramH, "u_Tint");
    if (mTintLoc < 0) {
        ALOGE("*** Couldn't get Tint attrib location from shader (OurShader).");
        ABORT_GAME;
    }
    mSamplerLoc = glGetUniformLocation(mProgramH, "u_Sampler");
    if (mSamplerLoc < 0) {
        ALOGE("*** Couldn't get sampler location from shader (OurShader).");
        ABORT_GAME;
    }
    mTexCoordLoc = (GLuint) glGetAttribLocation(mProgramH, "a_TexCoord");
    if (mTexCoordLoc < 0) {
        ALOGE("*** Couldn't get tex coord attribute location from shader (OurShader).");
        ABORT_GAME;
    }
    mPointLightPosLoc = glGetUniformLocation(mProgramH, "u_PointLightPos");
    if (mPointLightPosLoc < 0) {
        ALOGE("**** Couldn't get point light position uniform (OurShader).");
        ABORT_GAME;
    }
